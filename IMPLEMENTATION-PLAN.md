...
- Backups: infra-owned tooling shared by every project (daily DB dump + media archive, weekly restore drill on the server, age check that fails the run); backup files outside the project tree; this repository only calls the pre-deploy dump (0.9).
...
| 0.9       | Backups               | Adopt the infra-owned backup tooling (`/opt/shared/backup`, files under `/var/backups/<project>/<env>`): register the project in infra, call the pre-deploy dump from 0.8, verify the first scheduled run, drill and age check; nothing backup-related in this repo — `docs/phase-0-design.md` 0.9 | Scheduled dump + media archive exist, restore drill passes, age check fails the run    |
...
| Iteration | Objective                           | Scope                                                                                                                                                                                       | Acceptance                                        |
| --------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| 1.1       | Auth schema                         | Port `User` (minus currency, plus `avatarMediaId`, `bio`, `mediaQuotaBytes`, `mediaUsedBytes`), `RefreshToken`, `OAuthProvider`, `EmailVerificationToken`, `PasswordResetToken`, `AuditLog` | Migration applied on staging                      |
