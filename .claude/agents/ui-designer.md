...
`wiki/ui-design-system.md` (principles, tokens, page templates), the "Frontend Pages and
Components" section of the phase design doc, `apps/web/src/components/**` (what exists),
`apps/web/src/app/globals.css` (tokens), `apps/web/messages/en.json` (key namespaces).
...
4. Components: reuse from `components/ui` and domain folders first; new ones named with props.
...
in the same change so the system stays the single source. Run prettier on written files.
