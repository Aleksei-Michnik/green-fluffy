---
name: infra-sync
description: Read-only procedure for probing the sibling infra and mrmichnik repos for changes that affect this project and refreshing wiki/infra-context.md. Use when the wiki sync date is stale (>7 days), before deploy/compose/CI/nginx/Mdocker work, or when asked what the infra side is doing.
---

# infra-sync

Repos (private, local clones, **never modify**): `~/Aleksei-Michnik/infra` (plan, edge, mail,
DNS, Mdocker), `~/mrmichnik` (WordPress sites — newest reference implementation of the shared
...
3. Read, in order: `infra/docs/notes/next-session.md` (top + the step table),
   `infra/docs/notes/phase-2-pipeline.md` §1, `infra/mdocker/hosts.json` (+ README if present),
...
4. Extract only what changes green-fluffy's work: Mdocker adoption shape, edge contract, deploy
